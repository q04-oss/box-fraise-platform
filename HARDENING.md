...
| App Attest full-crypto verify in `record_beacon_dwell` / `record_nfc_tap`. Header-presence enforcement landed via `AppAttestPolicy::ENABLED`; per-device public-key DER storage at registration time still pending. | §3a | `domain/src/auth/apple_attest.rs::enforce_assertion` (TODO `app-attest-full`) |
