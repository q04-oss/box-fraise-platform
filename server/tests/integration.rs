...
async fn create_business_end_to_end(pool: PgPool) {
    ...
    let resp = with_rls_tx!(&pool, user_id, |tx| {
        service::create_business(&mut tx, &pool, user_id, req, &bus)
            .await
...
async fn create_beacon_end_to_end(pool: PgPool) {
    ...
    let bus      = EventBus::new();
    let biz_resp = with_rls_tx!(&pool, user_id, |tx| {
    ...
    let beacon_resp = with_rls_tx!(&pool, user_id, |tx| {
        beacon_service::create_beacon(
            &mut tx,
            &pool,
            user_id,
            CreateBeaconRequest {
    ...
    let uuid_resp = with_rls_tx!(&pool, user_id, |tx| {
        beacon_service::get_daily_uuid(&mut tx, &pool, beacon_resp.id, user_id)
            .await
...
async fn full_background_check_journey(pool: PgPool) {
    ...
    let sanctions = with_rls_tx!(&pool, user_id, |tx| {
    ...
    let fraud = with_rls_tx!(&pool, user_id, |tx| {
        service::initiate_check(
            &mut tx, &pool, user_id,
...
async fn full_staff_visit_journey(pool: PgPool) {
    ...
    let visit = with_rls_tx!(&pool, admin, |tx| {
    ...
    // ── Step 3: arrive ────────────────────────────────────────────────────────
    let arrived = with_rls_tx!(&pool, admin, |tx| {
        service::arrive_at_visit(
    ...
    let assessment = with_rls_tx!(&pool, admin, |tx| {
    ...
    let completed = with_rls_tx!(&pool, admin, |tx| {
...
async fn full_attestation_journey(pool: PgPool) {
    ...
    let after_staff_sign = with_rls_tx!(&pool, staff, |tx| {
    ...
    let after_r1 = with_rls_tx!(&pool, r1, |tx| {
    ...
    let approved = with_rls_tx!(&pool, r2, |tx| {
        attest_svc::reviewer_sign(
...
async fn full_soultoken_lifecycle(pool: PgPool) {
    ...
    let token = with_rls_tx!(&state.db, uid, |tx| {
    ...
    let renewal = with_rls_tx!(&state.db, uid, |tx| {
        st_svc::renew_soultoken(
            &mut tx, &state.db, UserId::from(uid),
    ...
    // ── Step 3: Revoke ────────────────────────────────────────────────────────
    with_rls_tx!(&state.db, admin_id, |tx| {
...
async fn full_support_booking_journey(pool: PgPool) {
    ...
    let visit = with_rls_tx!(&pool, staff, |tx| {
        staff_svc::schedule_visit(&mut tx, &pool, staff,
            ScheduleVisitRequest {
                location_id:              loc_id,
    ...
    let booking = with_rls_tx!(&pool, user, |tx| {
        sup_svc::create_booking(&mut tx, &pool, user,
    ...
    let attended = with_rls_tx!(&pool, staff, |tx| {
        sup_svc::attend_booking(&mut tx, &pool, booking.id, staff)
    ...
    let resolved = with_rls_tx!(&pool, staff, |tx| {
    ...
    let visit2 = with_rls_tx!(&pool, staff, |tx| {
        staff_svc::schedule_visit(&mut tx, &pool, staff,
            ScheduleVisitRequest {
                location_id:              loc_id,
                visit_type:               "support".to_owned(),
    ...
    let booking2 = with_rls_tx!(&pool, user, |tx| {
        sup_svc::create_booking(&mut tx, &pool, user,
...
async fn full_attestation_token_lifecycle(pool: PgPool) {
    ...
    let issued = with_rls_tx!(&pool, uid, |tx| {
...
async fn full_audit_trail_completeness(pool: PgPool) {
    ...
    let trail = with_rls_tx!(&pool, uid, |tx| {
        ve_svc::get_my_audit_trail(&mut tx, &pool, user)
...
async fn full_configuration_lifecycle(pool: PgPool) {
    ...
    let all = with_admin_tx!(&pool, |atx| {
        cfg_svc::get_all_configuration(&mut atx, &pool, admin).await.unwrap()
    });
    ...
    let updated = with_admin_tx!(&pool, |atx| {
