...
pub async fn find_by_id(pool: &PgPool, id: UserId) -> AppResult<Option<UserRow>> {
    // Wrap the SELECT in a tx that sets `app.user_id` to the looked-up
    // user's id. Under app_user (RLS active), the `users_self` policy
    // permits reading WHERE `id = app.user_id` — every caller of
    // `find_by_id` is asking "look up user X" where X is either the
    // requesting user or an admin checking their own id, so the scope
    // matches the WHERE clause. Under fraise (BYPASSRLS) this is a
    // harmless no-op.
    let id_i = i32::from(id);
...
pub async fn find_by_email(pool: &PgPool, email: &str) -> AppResult<Option<UserRow>> {
    // Email lookup runs pre-auth (magic link request, password reset). The
    // requester's user_id is unknown — the *result* of the lookup is what
    // identifies them. So we run under admin scope (`SET LOCAL ROLE app_admin`)
    // for the duration of the SELECT. This is safe because: (a) every caller
    // is itself a privileged operation by construction, and (b) the function
    // returns a UserRow, not raw column data, so the caller picks up RLS
    // context for subsequent writes via `set_rls_user_context`.
    let mut tx = pool.begin().await.map_err(DomainError::Db)?;
...
pub async fn find_or_create_apple(
    ...
    let mut tx = pool.begin().await.map_err(DomainError::Db)?;
    // Pre-auth: the user is identifying via Apple ID. Run under admin role so
    // the user lookup/insert can see/write the row regardless of RLS context.
    set_rls_admin_context(&mut tx).await.map_err(DomainError::Db)?;
...
pub async fn find_or_create_magic_link_user(
    ...
    // Pre-auth INSERT — wrap in admin tx so RLS doesn't reject. See
    // `find_by_email` for the rationale.
    let mut tx = pool.begin().await.map_err(DomainError::Db)?;
    set_rls_admin_context(&mut tx).await.map_err(DomainError::Db)?;
...
pub async fn set_push_token(pool: &PgPool, user_id: UserId, token: &str) -> AppResult<()> {
    ...
    sqlx::query("SELECT set_config('app.user_id', $1, true)")
        .bind(uid.to_string())
        .execute(&mut *tx).await.map_err(DomainError::Db)?;
...
pub async fn set_verified(pool: &PgPool, user_id: UserId) -> AppResult<()> {
    // Magic-link verification: the user just proved possession of their email,
    // but the verifying request is pre-JWT so `app.user_id` isn't set on the
    // pool. Use admin scope for the UPDATE.
    let mut tx = pool.begin().await.map_err(DomainError::Db)?;
    set_rls_admin_context(&mut tx).await.map_err(DomainError::Db)?;
